...
module.exports = {
  add_gestionColis: async (req, res) => {
    try {
      ...
      const savedgestionColis = await GestionColis.create(gestionColis);
  ...
  add_many_gestionColis: async (req, res) => {
    try {
      ...
      const savedgestionColis = await GestionColis.insertMany(gestionColis);
      res.send(savedgestionColis);
    } catch (error) {
      res.send(error);
    }
  ...
  update_one_gestionColis: async (req, res) => {
    try {
      ...
      const updatedgestionColis = await GestionColis.findByIdAndUpdate(
        id,
        gestionColis
      );
  ...
  delete_one_gestionColis: async (req, res) => {
    try {
      let id = req.params._id;
      const deletedgestionColis = await GestionColis.findByIdAndRemove(id);
